...
- **강조색은 마젠타 `#ff3bff` 하나다.** 씬마다 가장 중요한 단어 **하나**에만 찍는다.
- FRAME.md의 `accent_promise`(옐로/골드)는 **2026-07-30 폐지된 2색 체계의 잔재**다. 쓰려면 그 기수 `## 디렉터 합의`에 명시해야 한다. 기본값은 미사용.
  (`magenta-block`은 마젠타가 배경이라 대비색이 필요한 변주 — 이 경우만 예외.)
- 전환·약속은 색이 아니라 카피·연출(취소선·밑줄·모션)이 진다.

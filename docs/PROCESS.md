...
| 2 | content.md | `landing-builder` | `app/{course}/page.tsx` + `courses/{course}/imweb.html` + `visual-brief.md` | `npm run dev`로 브라우저 확인 |
...
## Phase 2 — 랜딩 빌드 (Next.js + 아임웹)

**목표:** 확정된 콘텐츠를 **Next.js 페이지와 아임웹 HTML로 동시 생성**. 이미지는 placeholder 경로로 두고 시각 디렉션을 별도 산출.
...
1. `app/{course}/page.tsx` — Next.js 14 App Router 페이지 (`app/classic/page.tsx` 디자인 시스템 따름)
2. `courses/{course}/imweb.html` — 아임웹 코드블럭용 HTML (인라인 CSS·vanilla JS)
3. `courses/{course}/visual-brief.md` — 섹션별 이미지 가이드 (Phase 3 입력)

### 검토 게이트

```bash
npm run dev   # http://localhost:3000/{course}
```

- [ ] 모바일 375px에서 레이아웃 깨짐 없음
- [ ] `<h1>` 1개만, `next/image` 사용 (img 태그 금지)
- [ ] 메타 데이터(`generateMetadata`) 설정됨
- [ ] CTA 버튼이 above-the-fold에 있음
...
| `landing-builder` | 2 | content.md | page.tsx + imweb.html + visual-brief.md |
...
| `landing-optimizer` | 7 | analytics/output/ + page.tsx | 페이지 수정 PR |
...
mkdir -p courses/$COURSE/imweb courses/$COURSE/campaigns
mkdir -p public/courses/$COURSE
cp courses/_templates/USP.template.md courses/$COURSE/USP.md

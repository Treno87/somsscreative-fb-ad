...
> **선행 관문:** 카피 작성 전에 `ad-director` 스킬(디렉터 인터뷰)로 컨셉·앵글·제작 경로를
> 합의한다 — `/make-ad` 1단계. 합의 기록(`campaigns/{기수}.md`의 `## 디렉터 합의`)이
> 이후 `ad-proposal`·소재 생성의 구속 조건이다.
...
> **먼저 `creative-critic`으로 카피를 채점한다** → 사람은 그 진단을 보고 컨펌한다. 품질 판정을 눈이 아니라 채점표(`.claude/creative-rubric.md`)로 옮기는 게이트다.
> FAIL이 있으면 **합의와 충돌하는지 먼저 확인**하고, 충돌이면 소재가 아니라 합의서를 갱신한다. 충돌이 아니면 리라이트안 반영 후 재채점.

- [ ] `creative-critic` 채점에서 **미해소 FAIL 0건**인가? (합의서 `적용 제외 규칙`에 등재된 `면제`는 FAIL로 세지 않는다)
- [ ] 판정이 `합의 확인 필요`면 사용자 결정을 받고 `## 디렉터 합의`를 갱신했는가?
...
> 크리틱 FAIL이 광고주의 의도된 결정과 충돌하면 **소재를 고치지 말고 합의서를 먼저 갱신**한다.
> 전역 정본(`creative-rubric.md`·`PHILOSOPHY.md`)은 같은 예외가 3기수 연속 나올 때만 고친다.

- [ ] `creative-critic` 채점에서 **미해소 FAIL 0건**인가? (`면제`는 제외)
...
> **동영상 광고**(씬 구분·카피·이미지/영상·배경음악·텍스트 효과)는
> `docs/video-ad-process.md`(공통 관문)를 따른다. 제작 경로는 둘이며
> 관문 문서의 기준표로 선택한다 — **Remotion**(`docs/video-remotion.md`, JSON 주문서 →
> `remotion/` 렌더)과 **Hyperframe**(`docs/video-hyperframe.md`, HTML+GSAP →
> `ad-projects/{course}-{기수}-hf/`). 어느 경로든 씬 기획 승인·배달·마감은 공통이다.
